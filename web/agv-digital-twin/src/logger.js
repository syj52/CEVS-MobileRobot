...
class Logger {
    ...
    _write(level, message) {
        ...
        if (!this._list) return;
        this._pending.push({ level, time, message });
    ...
    _flush() {
        this._flushScheduled = false;
        if (!this._list || this._pending.length === 0) return;

        const frag = document.createDocumentFragment();
        for (const { level, time, message } of this._pending) {
            const entry = document.createElement('div');
            entry.dataset.level = level;
            const visible = this._filters[level] !== false;
