...
import { MqttRobot } from './mqtt_robot.js';
...
export class NavigationDisplay {
    ...
    async _init() {
        try {
            await this._loadMap();
            this._addRobot();
            // 对接 MQTT broker，让方块充当"小车端"
            this.mqttRobot = new MqttRobot({
                robotMesh: this.robotMesh,
                logger: this.log,
                onNavGoal: (msg) => {
                    // 收到导航目标 — 更新小地图
                    if (msg.target_coords) {
                        const gridPos = this._worldToGrid(
                            new THREE.Vector3(msg.target_coords.x, 0.2, msg.target_coords.z),
                        );
                        this._miniMapGoal = gridPos;
                        this._miniMapPath = null;
                        this._updateMiniMap();
                    }
                },
    ...
    async _loadMap() {
        ...
        const yamlText = await fetch(this.mapYamlUrl + "?v=" + timestamp).then(r => r.text());
        const yaml = this._parseYAML(yamlText);
        const pgmData = await fetch(this.mapPgmUrl + "?v=" + timestamp).then(r => r.arrayBuffer());
    ...
    // ---------------------------------------------------------------------------
    // 外部调用接口：由 main.js 在 LLM 导航指令时触发
    // ---------------------------------------------------------------------------

    /**
     * 导航到世界坐标点（供 LLM 导航调用）
     * @param {number} worldX Three.js 世界 X
     * @param {number} worldZ Three.js 世界 Z
     * @param {string} [poiName] 目标名称（日志用）
     */
    navigateTo(worldX, worldZ, poiName) {
        if (!this.mapData || !this.robotMesh) {
            this.log?.warn('导航不可用：地图或机器人未就绪');
            return;
        }
        const start = this._worldToGrid(this.robotMesh.position);
        const goal = this._worldToGrid(new THREE.Vector3(worldX, 0.2, worldZ));

        const { width, height, grid, maxval, negate, occupied_thresh } = this.mapData;
        if (goal.x < 0 || goal.x >= width || goal.y < 0 || goal.y >= height) {
            this.log?.warn(`目标超出地图范围 (${goal.x}, ${goal.y})`);
            return;
        }

        // 检查目标是否在障碍物上
        const idx = goal.y * width + goal.x;
        let v = grid[idx];
        if (negate) v = maxval - v;
        if (v / maxval >= occupied_thresh) {
            this.log?.warn(`目标点 (${goal.x},${goal.y}) 在障碍物上，放在最近空闲格`);
            // 简单膨胀搜索：往四周找最近的空闲格
            const found = this._findNearestFree(goal);
            if (found) {
                goal.x = found.x;
                goal.y = found.y;
            } else {
                this.log?.error('附近无空闲位置');
                return;
            }
        }

        const tag = poiName ? `「${poiName}」` : '';
        const dist = this._heuristic(start, goal) * (this.mapData.resolution || 0.05);

        const path = this._astar(start, goal);
        if (path && path.length) {
            this.log?.status(`🎯 导航 ${tag}: (${start.x},${start.y}) → (${goal.x},${goal.y}) ≈ ${dist.toFixed(2)}m`);
            this._miniMapGoal = goal;
            this._miniMapPath = path;
            this._showPath(path);
            this._animateRobot(path);
            this._updateMiniMap();
            // 通过 MQTT 下发目标到仿真端（保持双端同步）
            this.mqttRobot?.publishNavGoal(worldX, worldZ, poiName);
        } else {
            this.log?.warn(`无路可至 ${tag} (${goal.x},${goal.y})`);
        }
    }

    _findNearestFree(goal) {
        const { width, height, grid, maxval, negate, occupied_thresh } = this.mapData;
        const maxRadius = 10; // 最多搜 10 格
        for (let r = 1; r <= maxRadius; r++) {
            for (let dx = -r; dx <= r; dx++) {
                for (let dy = -r; dy <= r; dy++) {
                    ...
                    let v = grid[idx];
                    if (negate) v = maxval - v;
                    if (v / maxval < occupied_thresh) return { x: nx, y: ny };
                }
            }
        }
        return null;
    ...
    _addRobot() {
        ...
        const bodyGeom = new THREE.BoxGeometry(res * 2.5, res * 1.2, res * 2.5);
        const bodyMat = new THREE.MeshBasicMaterial({ color: 0x00ffff, side: THREE.DoubleSide, depthTest: false });
        const body = new THREE.Mesh(bodyGeom, bodyMat);
        const group = new THREE.Group();
        group.add(body);
        const headGeom = new THREE.ConeGeometry(res, res, 3);
        const headMat = new THREE.MeshBasicMaterial({ color: 0xffff00 });
        const head = new THREE.Mesh(headGeom, headMat);
        head.rotation.x = Math.PI / 2;
        head.position.z = res * 1.5;
        group.add(head);
        this.robotMesh = group;
    ...
    _animateRobot(path) {
        ...
        const moveNext = () => {
            ...
            if (i >= path.length) { this._animating = false; this.mqttRobot?.publishPosition(); return; }
    ...
    _keyboardLoop() {
        ...
        // 每秒上报 ~5 次位置
        if (!this._lastPosPub || now - this._lastPosPub > 200) {
            this._lastPosPub = now;
            this.mqttRobot?.publishPosition();
        }
        requestAnimationFrame(this._keyboardLoopBound);
    }
    ...
    dispose() {
        ...
        // 断开 MQTT
        try { this.mqttRobot?.dispose(); } catch (e) {}
    }
}
