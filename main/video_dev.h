#ifndef VIDEO_DEV_H
#define VIDEO_DEV_H

#include <stddef.h>
#include <stdint.h>
#include <linux/videodev2.h>

#define CAM_DEV_PATH        ESP_VIDEO_MIPI_CSI_DEVICE_NAME
#define ENCODE_DEV_PATH     "/dev/video10"   /* JPEG HW encoder M2M */

/* Camera capture + JPEG encoder buffer counts */
#define BUFFER_COUNT        3
#define M2M_CAP_COUNT       3

typedef struct {
    uint8_t *buf;
    size_t   len;
    size_t   width;
    size_t   height;
} frame_buffer_t;

typedef struct {
    int      cap_fd;
    uint8_t *cap_buffer[BUFFER_COUNT];
    int      m2m_fd;                        /* JPEG encoder fd (/dev/video10) */
    uint8_t *m2m_cap_buffer[M2M_CAP_COUNT];
    frame_buffer_t fb;

    /* Pipeline state */
    bool     pre_fed;
    int      fed_cap_idx;
    int      last_enc_cap_idx;
    int      pending_requeue_idx;
    bool     first_call;
    bool     started;
} camera_context;

int          video_dev_init(camera_context *context);
esp_err_t    video_start(int width, int height, camera_context *cb_ctx);
frame_buffer_t *video_fb_get(camera_context *cb_ctx);
void         video_after_take(camera_context *cb_ctx);
void         video_stop(camera_context *cb_ctx);
void         video_get_resolution(int *w, int *h);

/* Expose shared I2C bus */
#include "driver/i2c_master.h"
i2c_master_bus_handle_t video_get_i2c_bus(void);

#endif
