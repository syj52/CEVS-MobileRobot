...
static void audio_capture_task(void *arg)
{
    ...
    ESP_LOGI(TAG, "Capture started (block=%u bytes / %d ms)",
             (unsigned)BLK_BYTES, AUDIO_BLOCK_MS);
    ...
    while (1) {
        ...
        while (total < BLK_BYTES) {
            ...
            /* esp_codec_dev_read blocks until all requested bytes are read */
            total = BLK_BYTES;
            (void)total;  /* suppress unused-variable if compiler complains */
        }
