/* streamer — HTTP server for audio streaming only.
 * Camera moved to RTSP (video_dev + rtsp_service). */
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
static esp_err_t wav_h(httpd_req_t *r){
    ...
    uint16_t bps = AUDIO_BITS;
    uint16_t ba = ch * (bps / 8);
    uint32_t brate = sr * ba;
    uint32_t data_sz = (uint32_t)pcm_len;
    uint32_t riff_sz = 36 + data_sz;

    uint8_t wav[44];
    memcpy(wav, "RIFF", 4);
    wav[4] = riff_sz & 0xFF; wav[5] = (riff_sz >> 8) & 0xFF;
    wav[6] = (riff_sz >> 16) & 0xFF; wav[7] = (riff_sz >> 24) & 0xFF;
    memcpy(wav + 8, "WAVE", 4);
    memcpy(wav + 12, "fmt ", 4);
    wav[16] = 16; wav[17] = 0; wav[18] = 0; wav[19] = 0;
    wav[20] = 1; wav[21] = 0;
    wav[22] = ch & 0xFF; wav[23] = (ch >> 8) & 0xFF;
    wav[24] = sr & 0xFF; wav[25] = (sr >> 8) & 0xFF;
    wav[26] = (sr >> 16) & 0xFF; wav[27] = (sr >> 24) & 0xFF;
    wav[28] = brate & 0xFF; wav[29] = (brate >> 8) & 0xFF;
    wav[30] = (brate >> 16) & 0xFF; wav[31] = (brate >> 24) & 0xFF;
    wav[32] = ba; wav[33] = 0;
    wav[34] = bps; wav[35] = 0;
    memcpy(wav + 36, "data", 4);
    wav[40] = data_sz & 0xFF; wav[41] = (data_sz >> 8) & 0xFF;
    wav[42] = (data_sz >> 16) & 0xFF; wav[43] = (data_sz >> 24) & 0xFF;
...
static esp_err_t status_h(httpd_req_t*r){
    ...
    char j[128]; int n=snprintf(j,sizeof(j),
        "{\"mic\":%d,\"rtsp\":8554}",
        audio_mic_is_ready()?1:0);
    httpd_resp_send(r,j,n); return ESP_OK;
}
